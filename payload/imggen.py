...
def save_png_bytes(image: Image.Image, mono: bool, dpi: int = 96, bit_depth: Optional[int] = None) -> bytes:
    buf = io.BytesIO()
    dpi = max(1, min(2400, int(dpi)))
    if bit_depth is None:
        bit_depth = 1 if mono else 4
    if bit_depth == 24:
        out = image.convert("RGB")
        out.save(buf, format="PNG", optimize=True, dpi=(dpi, dpi))
    elif bit_depth == 1:
        out = render_monochrome(image)
        out.save(buf, format="PNG", optimize=True, bits=1, dpi=(dpi, dpi))
    elif bit_depth == 4:
        out = render_indexed_16(image)
        out.save(buf, format="PNG", optimize=True, bits=4, dpi=(dpi, dpi))
    else:
...
def thumb():
    try:
        ...
        dpi = int(dpi_param) if dpi_param is not None and dpi_param != "" else 96
        if dpi < 1 or dpi > 2400:
            raise ValueError("DPI out of range.")
        bit_depth_param = request.args.get("bitdepth") or request.args.get("bit_depth")
